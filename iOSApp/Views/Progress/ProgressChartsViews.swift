...
struct MomentumChipsCard: View {
    ...
    @State private var showFullMomentum = false
    ...
    var body: some View {
        ...
        .contentShape(Rectangle())
        .onTapGesture { showFullMomentum = true }
        .sheet(item: $detailAnalytics) { ea in
            MomentumDetailSheet(analytics: ea)
        }
        .sheet(isPresented: $showFullMomentum) {
            NavigationStack {
                MomentumDetailView()
                    .navigationTitle("Momentum — All Exercises")
                    .navigationBarTitleDisplayMode(.inline)
                    .toolbar {
                        ToolbarItem(placement: .confirmationAction) {
                            Button("Done") { showFullMomentum = false }
                        }
                    }
            }
        }
...
struct PlateauRiskCard: View {
    ...
    private func sessionsSincePeak(_ ea: ExerciseAnalytics) -> Int {
        let e1rms = ea.sessions.map(\.estimated1RM)
        guard let maxIdx = e1rms.enumerated().max(by: { $0.element < $1.element })?.offset else { return 0 }
        return e1rms.count - 1 - maxIdx
    }

    private func riskFactors(_ ea: ExerciseAnalytics) -> String {
        var factors: [String] = []
        if ea.isPlateau { factors.append("slope flat ≥4 sessions") }
        else if ea.slopePerWeek < 0 { factors.append(String(format: "e1RM declining %.1f kg/wk", ea.slopePerWeek)) }
        else if ea.slopePerWeek < 0.3 { factors.append(String(format: "slow progress +%.1f kg/wk", ea.slopePerWeek)) }
        let recentE1rms = ea.sessions.suffix(4).map(\.estimated1RM)
        if recentE1rms.count >= 4 {
            let spread = (recentE1rms.max() ?? 0) - (recentE1rms.min() ?? 0)
            if spread < 2.5 { factors.append(String(format: "±%.1f kg variance over last 4", spread)) }
        }
        return factors.isEmpty ? "monitoring" : factors.joined(separator: ", ")
    }
            ...
            VStack(alignment: .leading, spacing: 12) {
                VStack(alignment: .leading, spacing: 2) {
                    Text("Plateau Risk")
                        .font(.system(size: 15, weight: .bold))
                        .foregroundColor(.primary)
                    Text("e1RM velocity · session variance · frequency")
                        .font(.system(size: 12))
                        .foregroundColor(.secondary)
                }
                    ...
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(spacing: 10) {
                            ForEach(qualifiedItems) { ea in
                                ...
                                let lastE1RM = ea.sessions.last?.estimated1RM
                                let velocity = ea.slopePerWeek
                                let sessBack = sessionsSincePeak(ea)
                                ...
                                VStack(alignment: .leading, spacing: 5) {
                                    ...
                                    // Specific numbers (F-15)
                                    if let e1rm = lastE1RM {
                                        Text(String(format: "e1RM %.1f kg", e1rm))
                                            .font(.system(size: 9, weight: .medium, design: .monospaced))
                                            .foregroundColor(.primary)
                                    }
                                    let velColor: Color = velocity >= 0.3 ? HONTheme.positive : velocity >= 0 ? HONTheme.warning : HONTheme.negative
                                    Text(String(format: "%+.2f kg/wk", velocity))
                                        .font(.system(size: 9, weight: .medium, design: .monospaced))
                                        .foregroundColor(velColor)
                                    if sessBack > 0 {
                                        Text("No PR: \(sessBack) sess")
                                            .font(.system(size: 8))
                                            .foregroundColor(sessBack >= 4 ? HONTheme.negative : .secondary)
                                    }
                                    let why = riskFactors(ea)
                                    Text(why)
                                        .font(.system(size: 7))
                                        .foregroundColor(.secondary)
                                        .lineLimit(3)
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack(spacing: 6) {
                    Text("Sets/wk:")
                        ...
                        .foregroundStyle(.secondary)
                    ForEach(
                        [(0, "0"), (3, "1–4"), (7, "5–9"), (14, "10–19"), (20, "20+")],
                        id: \.1
                    ) { sample, label in
                        HStack(spacing: 3) {
                            RoundedRectangle(cornerRadius: 2)
                                .fill(cellColor(sets: sample))
                ...
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Legend")
                            .font(.system(size: 14, weight: .semibold))
                        HStack(spacing: 12) {
                            legendSwatch(color: Color(.systemGray6).opacity(0.4), label: "0 sets")
                            legendSwatch(color: HONTheme.warning.opacity(0.25),   label: "1–4")
                            legendSwatch(color: HONTheme.chartSlate.opacity(0.45), label: "5–9")
                            legendSwatch(color: HONTheme.chartSage.opacity(0.65), label: "10–19")
                            legendSwatch(color: HONTheme.chartSage.opacity(0.9),  label: "20+")
                        }
                        HStack(spacing: 6) {
...
struct INOLCalendarDetailSheet: View {
    ...
    private func inolColor(_ inol: Double) -> Color {
        if inol == 0  { return Color(.systemGray6).opacity(0.2) }
        if inol < 0.4 { return HONTheme.positive.opacity(0.2) }
        if inol < 0.8 { return HONTheme.positive.opacity(0.5) }
        if inol < 1.4 { return HONTheme.chartSlate.opacity(0.7) }
        if inol < 2.0 { return HONTheme.warning.opacity(0.8) }
        return HONTheme.negative.opacity(0.9)
    }
                ...
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Zone Guide")
                            .font(.system(size: 14, weight: .semibold))
                        VStack(alignment: .leading, spacing: 8) {
                            inolZoneRow(color: HONTheme.positive.opacity(0.2),    label: "Low (<0.4)",          desc: "Warm-up / recovery session")
                            inolZoneRow(color: HONTheme.positive.opacity(0.5),    label: "Moderate (0.4–0.8)",  desc: "Good working volume — solid stimulus")
                            inolZoneRow(color: HONTheme.chartSlate.opacity(0.7),  label: "Optimal (0.8–1.4)",   desc: "Maximum adaptation zone — hit this regularly")
                            inolZoneRow(color: HONTheme.warning.opacity(0.8),     label: "Heavy (1.4–2.0)",     desc: "Use sparingly — high fatigue cost")
                            inolZoneRow(color: HONTheme.negative.opacity(0.9),    label: "Overreaching (>2.0)", desc: "Recovery debt — deload soon")
                        }
                    }
                    .padding(14)
                    .background(AppTheme.cardBG)
