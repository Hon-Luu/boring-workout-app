...
final class HealthKitService {
    ...
    var sleepHoursLast7: Double = 0   // avg hours/night over last 7 days
    var restingHeartRate: Double? = nil // most recent resting HR (bpm)
    ...
    func requestAndFetch() {
        ...
        // Skip the authorization prompt in simulator; treat as granted for UI testing
    ...
    // MARK: - Analytics-ready derived metrics

    private func fetchSleepHoursLast7() {
        Task { @MainActor in
            let points = await fetchDailySleepHistory(days: 7)
            guard !points.isEmpty else { return }
            sleepHoursLast7 = points.map(\.value).reduce(0, +) / Double(points.count)
        }
    }
