...
struct AMRAPSessionView: View {
    ...
    private var mainSessionContent: some View {
        VStack(spacing: 0) {
            ...
            if let w = currentExercise?.weight, w > 0 {
...
struct EMOMSessionView: View {
    ...
    private func hapticPulse(style: UIImpactFeedbackGenerator.FeedbackStyle = .medium) {
        guard emomHapticsEnabled else { return }
                ...
                VStack(spacing: 24) {
                    ...
                    HStack(spacing: 0) {
                        ...
                        if entry.isWeightedSession && entry.totalVolume > 0 {
                            Divider().frame(height: 40).background(HONTheme.textPrimary.opacity(0.2))
                            statCell(value: String(format: "%.0f", entry.totalVolume), label: "kg·reps")
                        }
                    }
                    .padding()
                            ...
                            HStack(spacing: 10) {
                                ForEach(FeelRating.allCases, id: \.self) { rating in
                                    Button(action: {
                                        ...
                                        store.updateCardioFeel(entryId: entry.id, feel: feelRating)
                                    }) {
                                        Text(rating.rawValue)
                                            ...
                                            .background(
                                                feelRating == rating
                                                    ...
                                                    : HONTheme.textPrimary.opacity(0.1),
                                                in: Capsule()
                                            )
                                            .foregroundStyle(feelRating == rating ? HONTheme.textPrimary : HONTheme.textPrimary.opacity(0.7))
                                    }
                                    .buttonStyle(.plain)
                                }
                            }
