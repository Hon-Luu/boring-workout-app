...
struct PatternDetailView: View {
    ...
    private var psiChartCard: some View {
        DetailCard(title: "Fiber Load Trend", icon: "waveform.path.ecg.rectangle", accent: accent) {
            VStack(spacing: 8) {
                ...
                Chart {
                    ...
                    ForEach(psiPoints.indices, id: \.self) { i in
                        LineMark(x: .value("Date", psiPoints[i].date),
                            ...
                            .symbol {
                                Circle().fill(accent).frame(width: 5)
                ...
                HStack(spacing: 14) {
                    HStack(spacing: 4) {
                        ...
                        Text("Raw fiber load").font(.system(size: 9)).foregroundStyle(.secondary)
                    }
                    HStack(spacing: 4) {
                        Rectangle().fill(accent.opacity(0.5)).frame(width: 14, height: 2)
