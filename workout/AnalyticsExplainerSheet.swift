...
private struct AEIntroCard: View {
    var body: some View {
        AECard(icon: "chart.line.uptrend.xyaxis", color: HONTheme.accent, title: "The Analytics Pipeline") {
            ...
            VStack(alignment: .leading, spacing: 8) {
                AE.step("1", "Raw sets → Estimated 1RM per set (Epley formula)")
...
private struct AEE1RMCard: View {
    var body: some View {
        AECard(icon: "scalemass.fill", color: HONTheme.warning, title: "Estimated 1RM (Epley Formula)") {
            AE.sectionLabel("What it is")
            ...
            AE.sectionLabel("Why it matters")
            AE.body("Testing a true 1RM is fatiguing, injury-prone, and impractical every session. Epley's formula extracts a consistent strength signal from normal training sets — a trackable number across weeks and months.")

            AE.sectionLabel("The formula")
            AE.latex(
                source: "e_{1RM} = w \\times \\left(1 + \\frac{r}{30}\\right)",
                readable: "e₁ᴿᴹ  =  w  ×  ( 1  +  r ÷ 30 )"
            )

            AE.sectionLabel("What each variable means")
            AE.component(symbol: "w", name: "Working weight", desc: "The load on the bar. The absolute foundation of the estimate.")
            AE.component(symbol: "r", name: "Reps completed", desc: "Actual reps performed. Most accurate between 3–12 reps. Sets outside this range are deprioritized in trend calculations.")
            ...
            AE.component(symbol: "1 + r/30", name: "The multiplier", desc: "How much heavier your e1RM is vs. working weight. At 1 rep: ×1.033. At 5 reps: ×1.167. At 10 reps: ×1.333.")

            AE.sectionLabel("Scenarios")
            AEScenario(emoji: "💪", label: "Standard hypertrophy set", color: HONTheme.warning,
                context: "Classic 3×8 at 80 kg — typical muscle-building rep range.",
                lines: ["w = 80 kg,   r = 8 reps",
                        "e₁ᴿᴹ = 80 × (1 + 8/30)",
                        "     = 80 × 1.267  =  101.3 kg"],
                verdict: "Session best = 101.3 kg")
            AEScenario(emoji: "🏋️", label: "Heavy strength set", color: HONTheme.negative,
                context: "100 kg × 3 reps. Low reps, near-max weight.",
                lines: ["w = 100 kg,  r = 3 reps",
                        "e₁ᴿᴹ = 100 × (1 + 3/30)",
                        "     = 100 × 1.100  =  110.0 kg"],
                verdict: "Higher e1RM despite fewer reps — heavier weight wins")
            AEScenario(emoji: "📊", label: "Multiple sets — best wins", color: HONTheme.accent,
                context: "Two top sets in one session. App picks the highest e1RM.",
                lines: ["Set A: 80 kg × 8  →  e₁ᴿᴹ = 101.3 kg",
                        "Set B: 85 kg × 5  →  85 × 1.167 = 99.2 kg",
                        "",
                        "Best = max(101.3, 99.2) = 101.3 kg"],
                verdict: "Set A wins. Session e1RM = 101.3 kg")
            AEScenario(emoji: "⚠️", label: "High-rep outlier — less reliable", color: .gray,
                context: "15 reps at 60 kg. Epley over-estimates at very high rep counts.",
                lines: ["e₁ᴿᴹ = 60 × (1 + 15/30) = 60 × 1.5 = 90.0 kg"],
                verdict: "Counted for volume but down-weighted in trend line")
        }
    }
}
    ...
    var body: some View {
        AECard(icon: "star.circle.fill", color: HONTheme.chartLavender, title: "Composite Strength Score (CSS)") {
            ...
            VStack(spacing: 0) {
                ...
                GradeRow(grade: "Recovering", range: "50–59",  meaning: "Plateau or recovery phase",              color: HONTheme.negative)
                GradeRow(grade: "Starting",   range: "< 50",   meaning: "Low stimulus — more data needed",        color: Color(.systemGray))
            }
            .background(AppTheme.insetBG, in: RoundedRectangle(cornerRadius: 10))
