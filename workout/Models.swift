...
struct WorkoutTemplate: Identifiable, Codable {
    ...
    // Custom decoder so circuitIds (added later) doesn't break existing stored data
    init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        id         = try c.decodeIfPresent(UUID.self,               forKey: .id)         ?? UUID()
    ...
    init(name: String = "My Routine", exercises: [TemplateExercise] = [], circuitIds: [UUID] = []) {
        self.id         = UUID()
        self.name       = name
...
struct UserProfile: Codable {
    var bodyWeightKg: Double?         // required for normalized PSI and relative strength
