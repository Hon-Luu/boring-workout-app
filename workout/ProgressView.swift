                    ...
                    if !store.isLoaded {
                        ...
                        if !store.cardioLog.isEmpty {
                            CollapsibleDashSection(
                                ...
                                CardioSectionContent(cardioLog: store.cardioLog)
                            }
                        }

                        CollapsibleDashSection(
                            title: "WHAT INFLUENCES ME",
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            if bodyWeightKg == nil {
                HStack(spacing: 8) {
                    ...
                    Text("Add body weight in Settings to see benchmarks.")
                        .font(.caption).foregroundStyle(.secondary)
                }
                .padding(.vertical, 10)
            } else if log.isEmpty {
                Text("Log workouts to see your benchmarks.")
                    .font(.caption).foregroundStyle(.secondary)
                    .padding(.vertical, 10)
            } else {
...
struct TierProgressBar: View {
    ...
    private var labelHeight: CGFloat {
        ...
        if hasKg && hasPct { return 36 }
        if hasKg || hasPct { return 26 }
        return 14
    }
                ...
                ExpandableChartCard(title: "8-Week Volume Trend") {
                    Chart(bars) { bar in
                        BarMark(x: .value("Week", bar.label),
                            ...
                            .foregroundStyle(bar.isCurrent ? HONTheme.accent : Color.blue.opacity(0.28))
                            .cornerRadius(3)
                    }
                    .chartXAxis {
...
private struct QuickStatsRow: View {
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            stat("\(streak)",    "Streak",    "flame.fill",                           HONTheme.warning)
        }
    }
