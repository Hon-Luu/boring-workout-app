            ...
            ZStack(alignment: .bottom) {
                ScrollView {
                    LazyVStack(spacing: 0) {
                        ...
                        .padding(.horizontal)
                        .padding(.bottom, restSecondsRemaining > 0 ? 100 : 16)
                    }
                    .padding(.top, 8)
                }
                ...
                if restSecondsRemaining > 0 {
                    ...
                    .animation(.spring(response: 0.4), value: restSecondsRemaining > 0)
                }
            }
...
private struct ExerciseCard: View {
    ...
    private var equipmentBadge: String? {
        switch equipment {
        case .dumbbell: return "× 2  (enter per-hand weight)"
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 6) {
                ...
                // Complete / unlog
                Button(action: record.isCompleted ? onUncomplete : onComplete) {
        ...
        .background(
            record.isCompleted
                ? (record.repOutcome == .missed ? Color.orange.opacity(0.05) : Color.green.opacity(0.06))
                : isActive ? Color.blue.opacity(0.04) : Color.clear
        )
...
private struct RestTimerBanner: View {
    ...
    private var progress: Double { 1.0 - Double(seconds) / Double(duration) }
...
private struct SupersetConnector: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            Rectangle().fill(HONTheme.warning.opacity(0.5)).frame(width: 3).padding(.leading, 36)
            HStack(spacing: 4) {
...
private struct HistoryCell: View {
    ...
    var body: some View {
        Text(set.weight.weightFormatted)
            ...
            .frame(width: 54, alignment: .center)
            .padding(.vertical, 5)
            .background(hitTarget ? Color.green.opacity(0.18) : Color.red.opacity(0.18),
