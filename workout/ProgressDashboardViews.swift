...
struct DashboardHeroCard: View {
    ...
    private var strengthStatChips: some View {
        HStack(spacing: 0) {
            ...
            statChip("\(streak)🔥", "Streak", "flame.fill", HONTheme.warning)
            Divider().frame(height: 32)
            statChip("\(strengthScore?.coveredCategories.count ?? 0)/3", "Lifts", "checkmark.circle.fill", HONTheme.positive)
            Divider().frame(height: 32)
            statChip(momentumLabel, "Momentum", "bolt.fill", HONTheme.chartLavender)
        }
        .padding(.vertical, 12)
    }
    ...
    private var momentumLabel: String {
        ...
        if m >= 70 { return "↑ \(Int(m.rounded()))" }
        if m >= 45 { return "→ \(Int(m.rounded()))" }
...
struct WhereSectionContent: View {
    ...
    private var patternRadarCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Movement Pattern Strength")
                .font(.cardTitle)
            PatternRadarView(categoryAnalytics: analytics.categoryAnalytics)
            Text("Cumulative improvement rate across all sessions. Outer ring = Elite.")
                .font(.appFootnote)
                .foregroundStyle(.secondary)

            if analytics.categoryAnalytics.contains(where: { $0.improvementRatePerWeek <= 0 }) {
                let weak = analytics.categoryAnalytics
    ...
    private var muscleData: [MuscleItem] {
        ...
        return [
            MuscleItem(name: "H. Push",   rateLabel: push.0, color: push.1),
            MuscleItem(name: "V. Pull",   rateLabel: pull.0, color: pull.1),
            MuscleItem(name: "V. Push",   rateLabel: ohp.0,  color: ohp.1),
            MuscleItem(name: "Squat",     rateLabel: sq.0,   color: sq.1),
            MuscleItem(name: "Hinge",     rateLabel: dl.0,   color: dl.1),
...
struct PatternRadarView: View {
    ...
    private var axes: [Axis] {
        let patterns: [(MovementPattern, String)] = [
            (.horizontalPush, "H. Push"),
            (.verticalPull,   "V. Pull"),
            (.verticalPush,   "OHP"),
            (.hipHinge,       "Hinge"),
            (.kneeFlexion,    "Squat"),
            (.horizontalPull, "H. Pull"),
        ]
    ...
    var body: some View {
        Canvas { ctx, size in
            ...
            ctx.fill(dataPath, with: .color(.blue.opacity(0.2)))
        ...
        .overlay(alignment: .center) {
            ...
            GeometryReader { geo in
                ...
                Text("ELITE")
                    ...
                    .foregroundStyle(HONTheme.chartAmber.opacity(0.55))
                    .kerning(0.5)
...
struct TrainingLoadCard: View {
    let inolExercises: [(String, Double)]
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            ForEach(inolExercises, id: \.0) { name, val in
...
struct StrongerSectionContent: View {
    ...
    var body: some View {
        VStack(spacing: 12) {
            ...
            // INOL surface — shows session training load for all exercises
            let inolExercises = exerciseAnalytics.compactMap { ea -> (String, Double)? in
                guard let v = ea.latestINOL else { return nil }
                return (ea.exercise.name, v)
            }
            if !inolExercises.isEmpty {
                ExpandableChartCard(title: "Training Load (INOL)") {
                    TrainingLoadCard(inolExercises: inolExercises, exerciseAnalytics: exerciseAnalytics)
                }
            }
                ...
                if isExpanded {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        Text("BALANCE ANALYSIS")
                            .font(.system(size: 9, weight: .bold)).foregroundStyle(.secondary).kerning(0.5)
                        HStack(spacing: 6) {
                            Image(systemName: ratio > 1.3 ? "exclamationmark.triangle.fill" : ratio < 0.7 ? "exclamationmark.triangle.fill" : "checkmark.circle.fill")
                                .font(.system(size: 10))
                                .foregroundStyle(ratio > 1.3 || ratio < 0.7 ? HONTheme.warning : HONTheme.positive)
                            Text(ratio == 0 ? "No push/pull data yet." :
                                 ...
                                 ratio < 0.7 ? "Pull:Push ratio is \(String(format: "%.1f", 1/ratio)):1 — more pulling than pushing. Balanced, but add horizontal push if you want more chest development." :
                                 "Push:Pull ratio is \(String(format: "%.1f", ratio)):1 — well balanced.")
                                .font(.system(size: 11)).foregroundStyle(.secondary)
                                .fixedSize(horizontal: false, vertical: true)
                        }
                        Text("Dominant pattern: \(dominant) (\(totalVol > 0 ? Int((patternVolumes[0].volume / totalVol * 100).rounded()) : 0)% of volume). WHY IT MATTERS: Volume imbalances over months create joint and posture issues. Aim for roughly equal push and pull volume.")
                            .font(.system(size: 10)).foregroundStyle(.secondary)
                            .lineSpacing(2).fixedSize(horizontal: false, vertical: true)
                    }
                }
...
struct RepRangeCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Chart(repBuckets) { bucket in
                BarMark(x: .value("Count", bucket.count), y: .value("Range", bucket.label))
                    ...
                    .annotation(position: .trailing) {
                        Text("\(bucket.count)").font(.system(size: 8)).foregroundStyle(.secondary)
                    }
            }
            .chartXAxis(.hidden)
            .expandingFrame(normal: 140, expanded: 260)
            ...
            if isExpanded {
                Divider().padding(.vertical, 4)
                let total = repBuckets.reduce(0) { $0 + $1.count }
                let dominant = repBuckets.max(by: { $0.count < $1.count })
                let goalMap: [String: String] = [
                    "1–3": "Max strength (CNS-focused, heavy singles/triples)",
                    "4–6": "Strength-hypertrophy (heavy compound work)",
                    "7–9": "Hypertrophy / strength blend (most effective zone)",
                    "10–12": "Hypertrophy (moderate load, metabolic stress)",
                    "13+": "Muscular endurance (high reps, lighter loads)"
                ]
                VStack(alignment: .leading, spacing: 6) {
                    Text("ZONE BREAKDOWN")
                        .font(.system(size: 9, weight: .bold)).foregroundStyle(.secondary).kerning(0.5)
                    if let dom = dominant, total > 0 {
                        ...
                        Text("\(pct)% of your sets are in the \(dom.label) rep range — \(goalMap[dom.label] ?? "general training").")
                            .font(.system(size: 11)).foregroundStyle(.secondary)
                            .fixedSize(horizontal: false, vertical: true)
                    }
                    ...
                    if singleZone {
                        HStack(alignment: .top, spacing: 6) {
                            Image(systemName: "exclamationmark.triangle.fill")
                                .font(.system(size: 9)).foregroundStyle(HONTheme.warning)
                            Text("You're training almost exclusively in one rep range. Varying across 4–12 reps develops both strength and hypertrophy and prevents adaptation stalls.")
                                .font(.system(size: 10)).foregroundStyle(.secondary)
                                .fixedSize(horizontal: false, vertical: true)
                        }
                    }
                    Text("WHY IT MATTERS: Different rep ranges stress different muscle fiber types and energy systems. Strength peaking requires low reps (1–5); maximum muscle growth spans 6–20 reps with sufficient effort.")
                        .font(.system(size: 10)).foregroundStyle(.secondary)
                        .lineSpacing(2).fixedSize(horizontal: false, vertical: true)
                }
            }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if isExpanded {
                ...
                Text("WHY IT MATTERS: Sets/min reflects rest period length and session efficiency. Strength training typically runs 0.5–0.8 (longer rests); hypertrophy 0.8–1.3 (shorter rests). Very high density may mean you're not resting enough for full recovery between sets.")
                    .font(.system(size: 10)).foregroundStyle(.secondary)
                    .lineSpacing(2).fixedSize(horizontal: false, vertical: true)
            }
        }
                ...
                .chartYAxis {
                    AxisMarks(values: .automatic(desiredCount: 3)) { val in
                        ...
                        if let v = val.as(Double.self) {
                            AxisValueLabel { Text("\(Int(v))m").font(.system(size: 9)) }
                        }
                    }
                }
                .frame(height: 100)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if hasAnyData {
                ...
                if isExpanded, !overallReadiness.isEmpty {
                    Divider().padding(.vertical, 2)
                    HStack(alignment: .top, spacing: 8) {
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            Text(overallReadiness).font(.system(size: 12)).foregroundStyle(.secondary)
                                .fixedSize(horizontal: false, vertical: true)
                            Text("WHY IT MATTERS: HRV is the most reliable readiness signal — low HRV before training correlates with reduced performance and slower recovery. Sleep below 7 h reduces muscle protein synthesis by up to 20%. Resting HR elevated >5 bpm above your baseline suggests incomplete recovery.")
                                .font(.system(size: 10)).foregroundStyle(.secondary)
                                .lineSpacing(2).fixedSize(horizontal: false, vertical: true)
                        }
                    }
...
struct FeelVolumeCard: View {
    ...
    private var feelPoints: [FPPoint] {
        ...
        return log.prefix(30).compactMap { entry -> FPPoint? in
            ...
            switch feel {
            case .tired:  fi = 1; color = HONTheme.negative
            case .normal: fi = 2; color = HONTheme.accent
            case .strong: fi = 3; color = HONTheme.positive
            }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if feelPoints.count >= 3 {
                ...
                if isExpanded, !correlationInsight.isEmpty {
                    Divider().padding(.vertical, 4)
                    HStack(alignment: .top, spacing: 8) {
                        Image(systemName: "lightbulb.fill").font(.system(size: 11)).foregroundStyle(HONTheme.chartAmber)
                        VStack(alignment: .leading, spacing: 4) {
                            Text(correlationInsight).font(.system(size: 12)).foregroundStyle(.secondary)
                                .fixedSize(horizontal: false, vertical: true).lineSpacing(2)
                            Text("HOW TO USE: On Tired days, still show up but aim for 80% effort. Research shows even reduced-volume sessions preserve more strength than skipping — and consistent attendance beats optimal-day training.")
                                .font(.system(size: 10)).foregroundStyle(.secondary)
                                .lineSpacing(2).fixedSize(horizontal: false, vertical: true)
                        }
                    }
