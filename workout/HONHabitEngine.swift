...
final class HONHabitEngine {
    ...
    // MARK: - Session Hook (called from WorkoutTabView on new session)
    ...
    private func rebuildRecord(from log: [WorkoutLogEntry]) {
        guard let firstDate = log.min(by: { $0.startedAt < $1.startedAt })?.startedAt else { return }
        let now = Date()
        let cal = Calendar.current
        ...
        userRecord.totalSessions = log.count

        // Build week records from first Monday through current Monday
        var records: [HONWeekRecord] = []
        var cursor = mondayOf(firstDate)
        let thisMonday = mondayOf(now)
        while cursor <= thisMonday {
            records.append(HONWeekRecord(weekStart: cursor, log: log))
        ...
        if sorted.count >= 2 {
            let gap = now.timeIntervalSince(sorted[0].startedAt) / 86_400
            userRecord.lapseStart = gap >= 7 ? sorted[0].startedAt : nil
        } else {
