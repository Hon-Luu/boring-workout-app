...
Each set row shows:
- **Target weight / target reps** — pre-filled from your last session or template (shown in the reps fraction field)
- **Actual weight** and **actual reps** — tap to edit directly
- **−2.5 / +2.5 buttons** flanking the weight field for quick increment/decrement
- **−1 / +1 buttons** flanking the reps field
- Tap the **checkmark circle** to mark the set complete

The app tracks whether you hit, exceeded, or missed your target reps:
- Green ✓ = hit or exceeded target
- Orange ✓ = missed target (fewer reps than target)
- Blue ↑ = exceeded target

### Weight conventions

| Equipment | Enter | Effective |
|---|---|---|
| **Dumbbell** | Weight per hand | App doubles it for analytics (×2) |
| **Barbell** | Total plate load (the bar is counted automatically; enter 0 for empty bar) | Minimum 20 kg (bar weight) |
| **All others** | Total weight as normal | Used as entered |

The column header shows **KG/HAND** for dumbbells as a reminder. A badge below the headers confirms the convention.

### To failure

Tap the **∞ icon** to the right of the reps field to mark a set as "to failure." The target reps display changes to a red **FAIL** pill. Completing a to-failure set always counts as hitting target regardless of reps.

### Drop sets

After completing a set (including a missed-target set), long-press the checkmark to access **Add Drop Weight**. This opens a panel where you can enter the lighter weight and reps for the drop portion of the set.

- Default drop reps are pre-filled with the gap between your target and actual reps (so you can note exactly what you finished with)
- Tap the green **✓** to confirm the drop
- Swipe left on a set to delete it
- The drop weight × reps is counted towards the set's total volume

The "Finish with lighter weight" nudge appears automatically at the bottom of any completed set that missed its target, as a shortcut to open the drop panel.

### RPE (Rate of Perceived Exertion)

After marking a set complete, a horizontal row of pills appears showing RPE values from **6.0 to 10.0** (in 0.5 steps):

| RPE | Meaning |
|---|---|
| 6 | Trivial — many reps left |
| 7 | Hard — several reps in reserve |
| 8 | Very hard — 2 reps in reserve |
| 9 | Near-max — 1 rep left |
| 10 | True max — no reps left |

The pills are color-coded: blue (6), green (7), orange (8–9), red (9.5–10). Tap a pill to log RPE; tap again to clear it. When RPE is logged, the analytics engine computes a **Zourdos RPE-adjusted e1RM** that accounts for reps-in-reserve, giving a more accurate estimate of true rested max.
...
The **Lab** tab (flask icon) is a research-grade view of your PCSA-weighted e1RM — strength normalized by the physiological cross-sectional area of the muscles recruited. It complements the CSS dashboard with a biomechanics lens.

---

### Hero card — PCSA-Weighted e1RM

The large number at the top is a single aggregate strength score computed as:
```
score = Σ (bilateralAdjustedE1RM_i × activationWeight_i) / Σ activationWeight_i
```

Where `activationWeight` = Σ (EMG% × PCSA) for each exercise. A deadlift contributes ~7× more than a lateral raise because it recruits far more muscle fiber mass.

**Delta arrow** — shows the week-over-week change. The arrow is suppressed if the change is within the Minimum Detectable Change (MDC) threshold for the movement pattern — changes within measurement noise are not shown.

**vs baseline** — percentage change from your early-training average (first 4 weeks). This is the primary long-term progress signal.

**Fresh capacity** — the fatigue-adjusted score, estimating what you'd be capable of fully rested.

---

### Controls

**Unit picker** (top left):

| Unit | What it shows |
|---|---|
| **kg** | Raw PCSA-weighted e1RM |
| **kg/cm²** | Normalized by total activation weight — removes body-size advantage |
| **N/cm²** | Specific force in physics units. A green band marks the in-vivo reference range of **15–22.5 N/cm²** (Erskine 2011) |

**Allometric toggle (÷ BW⁰·⁶⁷)** — divides by body weight raised to the 0.67 power, removing size advantage entirely. Disabled if no body weight is entered in Settings. All three unit modes can be combined with allometric scaling.

**90d / All** — time window for the trend charts.

**Training phase badge** — shows your current adaptation phase based on weeks since your first logged workout:

| Phase | Weeks | Meaning |
|---|---|---|
| Neural (0–4 wks) | 0–4 | Rapid gains from neural efficiency; hypertrophy minimal |
| Mixed (4–8 wks) | 4–8 | Neural + early structural adaptation |
| Hypertrophy (8+ wks) | 8+ | Structural gains; progress rate naturally slows |

---

### Pattern cards (Push / Pull / Legs)

Exercises are grouped into three movement categories. Each card shows:
- PCSA-weighted e1RM score for that group
- Week-over-week delta (MDC-gated: suppressed if change is within noise)
- Trend chart with the same unit as the global picker
- Contribution to overall score (% of total activation weight)

**Tap "Show exercises"** to expand per-exercise rows showing:
- Current e1RM and fresh (fatigue-adjusted) e1RM
- Fatigue gap (how much hidden capacity fatigue is masking)
- Peak all-time e1RM
- Δ/wk trend
- Mini chart (raw session dots + rolling average + fresh trend)

---

### N/cm² reference band

When the N/cm² unit is selected, a **green shaded band** at 15–22.5 N/cm² appears on all trend charts. This is the normal in-vivo specific force range for human skeletal muscle (Erskine 2011). Being within the band is normal; chronically below may indicate undertrained recruitment or high fat infiltration; significantly above reflects very high neural efficiency.
